...
/**
 * Default scoring: start from a difficulty-based budget and subtract penalties
 * for the number of moves used and the seconds elapsed, never dropping below a
 * floor. The weights are injectable, so the same algorithm can be re-tuned (OCP)
 * or replaced entirely with another {@link IScoringStrategy}.
 */
export class StandardScoringStrategy implements IScoringStrategy {
  ...
  score({ moves, elapsedMs, difficulty }: ScoreInput): Score {
    const base = this.config.baseByDifficulty[difficulty];
    const seconds = Math.floor(Math.max(0, elapsedMs) / 1000);
    const movesUsed = Math.max(0, moves);

    const raw =
      base - movesUsed * this.config.movePenalty - seconds * this.config.timePenaltyPerSecond;
